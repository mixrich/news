...
function FileUpload( $resourceType, $currentFolder, $sCommand, $CKEcallback = '' )
{
	...
	if (( isset( $_FILES['NewFile'] ) && !is_null( $_FILES['NewFile']['tmp_name'][0] ) ) || (isset( $_FILES['upload'] ) && !is_null( $_FILES['upload']['tmp_name'] ) ))
	{
		global $Config ;

		for($i=0;$i<count($_FILES['NewFile']['tmp_name']);$i++) {

			$FILES['name']     = $_FILES['NewFile']['name'][$i];
			$FILES['type']     = $_FILES['NewFile']['type'][$i];
			$FILES['tmp_name'] = $_FILES['NewFile']['tmp_name'][$i];
			$FILES['error']    = $_FILES['NewFile']['error'][$i];
			$FILES['size']     = $_FILES['NewFile']['size'][$i];

			$oFile = isset($FILES) ? $FILES : $_FILES['upload'];
	...
	if($CKEcallback == '')
	{
		SendUploadResults( $sErNum, $sFiUrl, $sFiNa ) ;
	}
	else
	{
...
function filemanager_thumb($IMAGE_SOURCE, $THUMB_X, $THUMB_Y, $IMAGE_OUT='') {
	...
	if (filemanager_imagemagick_check()) {
	    ...
	    2 => array("r"=>"jpeg", "w"=>"jpeg", "vr"=>"JPG Support", "vw"=>"JPEG Support"),
	  ...
	  $gd = gd_info();
     filemanager_debug("{$gd[$cmd['vr']]} {$gd[$cmd['vw']]} {$gd}");
	  if (!$gd[$cmd['vr']] && !$gd[$cmd['vw']]) {
	  	echo "Thumb - Формат не поддерживается PHP - ".$cmd['vr']." или ".$cmd['vw']."<br>";
	  	return false;
	  }
	  ...
	  if ($res)
	  	...
	  	file_put_contents($_SERVER['DOCUMENT_ROOT'].'/debug.txt'.'$res = Image'.$cmd['w'].'($DEST_IMAGE, $IMAGE_OUT);');
